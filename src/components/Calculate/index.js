...
const Calculate = () => {
    ...
    useEffect(() => {
        ...
        return () => {
            sr.destroy(); // Cleanup ScrollReveal instance on component unmount
        };
    }, []);
    ...
    return (
        <div id="calculadora" className="flex flex-col w-full h-full justify-center items-center py-20 px-5 relative" style={{ backgroundImage: `url(${background})`, backgroundPosition: 'bottom', backgroundRepeat: 'repeat' }}>
            ...
            <button onClick={() => setOpenModal(true)} className="lg:w-auto lg:h-auto mt-5 p-6 bg-gold rounded-lg shadow-2xl text-sm lg:text-lg relative z-10 transition duration-300 ease-in-out hover:scale-110">
